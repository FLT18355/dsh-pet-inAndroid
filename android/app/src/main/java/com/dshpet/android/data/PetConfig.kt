...
class PetConfig(private val ctx: Context) {
    ...
    companion object {
        ...
        private val K_MOUSE_THROUGH = booleanPreferencesKey("mouse_through") // 无法选中：桌宠不响应触摸（防止误触）；桌面端同键（WindowTransparentForInput）
    ...
    suspend fun setScale(v: Double) = set("scale", v.coerceIn(0.25, 2.0))
    ...
    suspend fun setMouseThrough(v: Boolean) = set("mouse_through", v)
